	...
	if (T)
	{
		...
		case ID:
			...
			if (symbolTable.symbols[rtn].flag == 'F')
				semantic_error(T->pos, T->type_id, "定义为函数名,类型不匹�?");
			else if (symbolTable.symbols[rtn].flag == 'A')
				semantic_error(T->pos, T->type_id, "定义为数组名,类型不匹�?");
			else
			{
				opn1.kind = ID;
	...
	if (T)
	{
		...
		case AUTOSUB:
			if (T->ptr[0])
			{
				...
				if (symbolTable.symbols[rtn].type != INT)
				{
					semantic_error(T->pos, "", "自减语句左值不为int�?");
					break;
				}
		...
		case COMMOD:
			if (T->ptr[0]->kind != ID && T->ptr[0]->kind != ARRAY_DF && T->ptr[0]->kind != ARRAY_CALL)
			{
				semantic_error(T->pos, "", "复合赋值语句需要左�?");
				break;
			}
			if (T->ptr[0]->type == CHAR)
			{
				semantic_error(T->pos, T->ptr[0]->type_id, "是字符类型变量，不能参与复合赋值运�?");
				break;
			}
			if (T->ptr[1]->type == CHAR)
			{
				semantic_error(T->pos, "", "字符类型变量不能参与复合赋值运�?");
				break;
			}
			switch (T->kind)
			{
		...
		case ARRAY_CALL:
			...
			if (rtn == -1)
			{
				semantic_error(T->pos, T->ptr[0]->type_id, "数组未定�?");
				break;
			}
			else if (symbolTable.symbols[rtn].flag == 'F')
			{
				semantic_error(T->pos, T->ptr[0]->type_id, "是函数名，不能采用下标访�?");
				break;
			}
			else if (symbolTable.symbols[rtn].flag == 'V')
			{
