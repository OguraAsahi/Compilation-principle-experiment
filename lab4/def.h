...
struct symbol {       //这里只列出了一个符号表项的部分属性，没考虑属性间的互斥
    ...
    char flag;       //符号标记，函数：'F'  变量：'V'   参数：'P'  临时变量：'T'，增加数组：“A”
...
//符号表,是一个顺序栈，index初始值为0
struct symboltable{
    struct symbol symbols[MAXLENGTH];
    int index;
    } symbolTable;
//每到达一个复合语句时，将符号表的index值进栈，离开复合语句时，取出其退栈值修改符号表的index，完成删除该复合语句中的所有变量和临时变量
...
void ArrayCall(struct ASTNode* T);
